          ...
          return NextResponse.json(
            {
              error:
                order.status === "pending_approval"
                  ? "Đơn hàng đang chờ Admin đối chiếu thanh toán. Quyền tải sẽ tự động mở sau khi duyệt."
                  : "Đơn hàng chưa thanh toán hoặc chưa được duyệt. Quyền truy cập bị từ chối.",
              status: order.status,
            },
            { status: 403 }
          );
